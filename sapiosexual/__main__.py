...
from sapiosexual.modules.helper_funcs.admin_rights import user_can_ban
from sapiosexual.modules.helper_funcs.readable_time import get_readable_time

from sapiosexual import (
    dispatcher,
    since_time_start,
    updater,
    TOKEN,
...
from sapiosexual.modules import ALL_MODULES
from sapiosexual.modules.purge import client
from sapiosexual.modules.helper_funcs.chat_status import is_user_admin
from sapiosexual.modules.helper_funcs.misc import paginate_modules
from sapiosexual.modules.helper_funcs.alternate import typing_action
...
Any issues or need help related to me? join our group [sapiosexual support chat](https://t.me/sapiosexualsupport).
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("sapiosexual.modules." + module_name)
...
def start(update, context):
    if update.effective_chat.type == "private":
        ...
        update.effective_message.reply_text(
            text="hey there! sapiosexual is alive :3"
...
def start_p(update, context):
    ...
    if query.data == "start_ping":
        sapiosexual_rbot = get_readable_time((time.time() - since_time_start))
        context.bot.answer_callback_query(query.id,
                                          text=f"sapiosexual is running since: {sapiosexual_rbot}"
                                               f"\n\nTelegram ping: {ping()}",
                                          show_alert=True)
...
@run_async
def sapiosexual_about_callback(update, context):
    query = update.callback_query
    if query.data == "aboutmanu_":
        query.message.edit_text(
            text="*sapiosexual is a bot for managing your group with additional features,*"
                 "\nand is fork of [marie](https://github.com/PaulSonOfLars/tgbot)."
                 "\n\n_sapiosexual's licensed under the GNU General Public License v3.0_,"
                 "\nhere is the [repository](https://github.com/gizmostuffin/sapiosexual)."
                 "\n\nIf any question about sapiosexual, let us know at @sapiosexualsupport.",
            parse_mode=ParseMode.MARKDOWN,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
        ...
        query.message.edit_text(
            text="*Basic help:*"
                 "\nTo add sapiosexual to your chats, simply click [here](http://t.me/zoldycktmbot?startgroup=true)  and select your chat."
        ...
        query.message.edit_text(
            text="<b>Admin permissions:</b>"
                 "\nTo avoid slowing down, sapiosexual caches admin rights for each user. This cache lasts about 10 minutes; this may change in the future. This means that if you promote a user manually (without using the /promote command), sapiosexual will only find out ~10 minutes later."
                 "\n\nIf you are getting a message saying:"
                 "\n<Code>You must be this chat administrator to perform this action!</code>"
                 "\nThis has nothing to do with sapiosexual's rights; this is all about YOUR permissions as an admin. sapiosexual respects admin permissions; if you do not have the Ban Users permission as a telegram admin, you won't be able to ban users with sapiosexual. Similarly, to change sapiosexual settings, you need to have the Change group info permission."
                 "\n\nThe message very clearly says that you need these rights - <i>not</i> sapiosexual.",
        ...
        query.message.edit_text(
            text="<b>Terms and Conditions:</b>"
                 ...
                 "\n- If you need to ask anything about this bot, Go @sapiosexualsupport."
                 "\n- If you asking nonsense in @sapiosexualsupport, you will get banned."
                 "\n- Sharing any files/videos others than about bot in @sapiosexualsupport is prohibited."
                 "\n- Sharing NSFW in @sapiosexualsupport will reward you banned/gbanned and reported to Telegram as well."
                 "\n\nFor any kind of help, related to this bot, Join @sapiosexualSupport."
                 "\n\n<i>Terms and Conditions will be changed anytime</i>"
                 "\nFollow @sapiosexualTM for Updates."
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton(
                            text="Support chat",
                            url="https://t.me/sapiosexualsupport",
                        )
                    ]
                ]
            ),
...
def main():
    ...
    about_callback_handler = CallbackQueryHandler(sapiosexual_about_callback, pattern=r"aboutmanu_")
