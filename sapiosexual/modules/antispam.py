...
import sapiosexual.modules.sql.antispam_sql as sql
...
from sapiosexual.modules.helper_funcs.chat_status import user_admin, is_user_admin
from sapiosexual.modules.helper_funcs.extraction import extract_user, extract_user_and_text, get_user
from sapiosexual.modules.helper_funcs.filters import CustomFilters
from sapiosexual.modules.helper_funcs.alternate import typing_action, send_action
from sapiosexual.modules.sql.users_sql import get_all_chats

from sapiosexual import client
from telethon import events

from html_telegraph_poster import TelegraphPoster
from html_telegraph_poster.upload_images import upload_image

sapiosexual_gban = TelegraphPoster(use_api=True)
sapiosexual_gban.create_api_token('sapiosexual gban', 'sapiosexual', 'https://www.meanii.me/') 
...
def gban(update, context):
    ...
    page = sapiosexual_gban.post(title=f"sapiosexual-gban-{user_chat.id}", author=f"{banner.first_name} ({banner.id})", text=EVIDENSE_NEW_GBAN)
    evidence_link = page.get('url')
    evidence_link = "<a href='{}'>{}</a>".format(evidence_link, f"sapiosexual GBanned // user_id: {user_chat.id}")
...
def __user_info__(user_id):
    ...
    if is_gbanned:
        ...
        if user.reason:
            ...
            text += "\n<i>- Appeal <a href={}>here</a>, if you think it's invalid.</i>".format('https://t.me/sapiosexualsupport')
    else:
