...
from sapiosexual.modules.helper_funcs.extraction import extract_user
...
from sapiosexual import dispatcher, SUDO_USERS, SUPPORT_USERS, LOGGER
from sapiosexual.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import sapiosexual.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `sapiosexual? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning`,`gm`,`good morning` | `goodnight`.

sapiosexual will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^sapiosexual\?"), decide, friendly="decide"
)
